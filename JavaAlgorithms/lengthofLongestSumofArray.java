/* Given two Boolean arrays X and Y, find the length of longest continuous
sequence that starts and ends at same index in both arrays and have same sum.
In other words, algorithm finds max(j-j+1) for every j >= i where sum of
sub-array X[i, j] is equal to sum of sub-array Y[i, j]. */
